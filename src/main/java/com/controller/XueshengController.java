    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(xuesheng !=null){
            ...
            BeanUtils.copyProperties( xuesheng , view );//把实体数据重构到view中
                ...
                //级联表
                YuanxiEntity yuanxi = yuanxiService.selectById(xuesheng.getYuanxiId());
                if(yuanxi != null){
                    BeanUtils.copyProperties( yuanxi , view ,new String[]{ "id", "createDate"});//把级联的数据添加到view中,并排除id和创建时间字段
                    view.setYuanxiId(yuanxi.getId());
                }
            //修改对应字典表字段
...
//                            xueshengEntity.setYuanxiId(Integer.valueOf(data.get(0)));   //院系 要改的
